...
local function move_picker_window(picker, x, y)
    ...
    elseif picker.resizing then
        ...
        if resize.corner:find("l") then
            left = math.max(0, math.min(right - 360, x))
        else
            right = math.min(sw, math.max(left + 360, x))
        end
        if resize.corner:find("t") then
            top = math.max(0, math.min(bottom - 300, y))
        else
            bottom = math.min(sh, math.max(top + 300, y))
        end
...
function JokerDisplay.draw_colour_picker()
    ...
    love.graphics.line(panel_x + panel_w - 4 - grip, panel_y + panel_h - 4, panel_x + panel_w - 4,
        panel_y + panel_h - 4 - grip)
    ...
    if picker then
        if button == 1 then
            ...
            for corner, rect in pairs(picker._resize_rects or {}) do
                if picker_contains(rect, x, y) then
                    resize_corner = corner; break
                end
            end
            ...
            elseif picker_contains(picker._back_rect, x, y) then
...
function love.mousemoved(x, y, dx, dy, ...)
    ...
    if picker and (picker.window_dragging or picker.resizing) then
        move_picker_window(picker, x, y)
    elseif picker and picker.dragging then
        picker_move(picker, x, y)
    end
    if picker then return end
